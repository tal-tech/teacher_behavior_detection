...
class HFBase(BaseModel):
    ...
    def demo_text_list(self, text_list):
        ...
        preds = self.k_filter(text_list, preds)
    ...
    def post_thresholds(self, input_preds):
        ...
        thresholds = [0.9999,  0.30,  0.99,   0.9,   0.95, 0.99,  0.99,   0.99, 0]
    ...
    def k_filter(self, input_text_list, preds):

        praise_prefix = ['真', '最', '非常', '太', '']
        praise_pattern = ['棒', '不错', '厉害', '牛了', '漂亮', '加油', '很好，', '很好。', '很好啊', '非常好', '太好', '真好']
        self._k_praise = list([i + j for (i, j) in list(itertools.product(praise_prefix, praise_pattern))])
        self._k_note = ['写', '记', '画', '笔', '题', '抄', '算', '答', '整', '拍']
        self._k_review = ['上', '复', '巩', '回', '想', '记', '前', '过']
        self._k_greet = '听看课听网好再节拜介休记卡'
        # self._k_conclude = '总结重点都概'

        self._k_not_praise = ['好的', '好了', '好吧']
        self._k_not_conclude = ['文章', '主旨', '中心', '结尾', '全文', '开头', '上文', '下文', '段']

        for idx, t in enumerate(input_text_list):
            try:
                if len(t) < 3:
                    preds[idx, 8] = preds[idx, 8] + 1.0

                if not self.k_is_in(t, self._k_greet):
                    preds[idx, 3] = preds[idx, 3] - 0.7

                if not self.k_is_in(t, self._k_praise):
                    preds[idx, 0] = preds[idx, 0] - 0.7

                if self.k_is_in(t, self._k_not_praise):
                    preds[idx, 0] = preds[idx, 0] - 0.7

                if self.k_is_in(t, self._k_not_conclude):
                    preds[idx, 2] = preds[idx, 2] - 0.7

                if not self.k_is_in(t, self._k_note):
                    preds[idx, 4] = preds[idx, 4] - 0.7

                if not self.k_is_in(t, self._k_review):
                    preds[idx, 6] = preds[idx, 6] - 0.7
            except:
                continue
        return preds
